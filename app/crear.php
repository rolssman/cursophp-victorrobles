...
if(isset($_POST["submit"])){
	...
	// mkdir:("crear el directorio", "permisos", "que sea recursi")
		if(!is_dir("uploads")){
		...
		if($dir){
			$filename = time()."-".$_FILES["image"]["name"];// tmp_name es el nombre temporal en el servidor para luego moverla a un dir y utilizarla
			$muf = move_uploaded_file($_FILES["image"]["tmp_name"], "uploads/".$filename);
			// move upload file
			$image = $filename; // actualiza la var $Image con $filename
